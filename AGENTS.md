...
- Asset bundling: `public/javascripts/yjs-dist.js` (core Yjs) and `public/javascripts/yjs-prosemirror.js` (y-prosemirror plugins) are generated via `bin/rake shared_edits:yjs:build` (`lib/tasks/yjs.rake` wraps `pnpm exec esbuild …`). Never hand-edit bundled files; re-bundle whenever `yjs`/`y-prosemirror` changes and commit the new artifacts.
